...
def PIMC(numSteps,Path):
    ...
    EnergyTrace     = []
    aglom           = 5
    leng            = int((numSteps)/equilSkip) #- 2
    rawLeng         = int(Path.numTimeSlices * numSteps)
    x               = np.zeros(leng*Path.numTimeSlices)
    z               = np.zeros(rawLeng)
    counts          = 0
    iter            = 0
    ...
    for steps in range(0,numSteps): 
        ...
        for i in range(Path.numTimeSlices):
            z[iter * Path.numTimeSlices + i] = x[i]
        iter += 1
        if steps % equilSkip == 0 and steps > equilSkip:
            for alph in range(Path.numTimeSlices):
                x[counts*Path.numTimeSlices + alph] = Path.beads[alph,0]
            counts += 1
    ...
    x_a = min(x)
    x_b = max(x)
    numBins = int((x_b - x_a)/binWidth)
    y = np.zeros(numBins)
    
    binLoc = 0
    for j in range(len(x)):
        binLoc = int((x[j] - x_a)/binWidth)
        if binLoc < 0:
            binLoc = 0
        elif binLoc >= numBins:
            binLoc = numBins -1
    ...
    with open(name,"a") as f:
        for k in range(len(y)):
            print(y[k],file=f)
    f.close()
    with open("sho_raw_x1.dat","a") as g:
